...
let wordList = [];
let solutionList = ['dark', 'fuel', 'open', 'cord', 'arch', 'reel', 'ping', 'jade', 'base', 'atom', 'balm', 'tack', 'root', 'meat', 'sack', 'bode', 'worn', 'pure', 'alas', 'thaw', 'task', 'stop', 'ugly', 'rind', 'glad', 'care', 'song']
...
function updateGrid() {
    for (let i = 0; i < 6; i++) {
        ...
        if (i < attempts.length) {
            drawAttempt(row, attempts[i], false, true)
        } else if (i === attempts.length) {
            drawAttempt(row, currentAttempt, true, false)
        } else {
...
function drawAttempt(row, attempt, isCurrent, isSolved) {
    for (let i = 0; i < 4; i++) {
        ...
        if (!isCurrent) {
            ...
            front.style.border = '0.04em solid white'
            back.style.backgroundColor = getBgColor(attempt,i)
            back.style.border = '0.04em solid transparent'
            ...
            if (attempt[i] !== undefined) {
                ...
                front.innerHTML = '<div style="opacity: 0">X</div>'
                front.style.borderColor = '#D3D3D3'
                back.innerHTML = '<div style="opacity: 0">X</div>'
                back.style.borderColor = '#D3D3D3'
                clearAnimation(cell)
            }
            front.style.backgroundColor = 'transparent'
...
function pauseInput() {
    if (isAnimating) throw Error('should not happed')
    isAnimating = true
    setTimeout(() => {
        isAnimating = false
    }, 2000)
